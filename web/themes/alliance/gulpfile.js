...
// По умолчанию эта папка называется projectName, поэтому её нужно переименовывать
...
function jsLibs() {
  return src(path.src.jsLibs).pipe(concat("libs.js")).pipe(dest(path.dist.js));
}
function jsMin() {
...
function videoDist() {
  return src(path.src.video).pipe(dest(path.dist.video));
}

function cleanDist() {
  return src(path.dist.allFiles).pipe(clean());
}

import * as nodePath from "path";
const repositoryName = nodePath.basename(nodePath.resolve());
const connect = {
  host: "188.225.40.227", // Адрес FTP сервера
  user: "wm84480", // Имя пользователя
...
function uploadFtp() {
  ...
  return src(`${path.dist.html}/**/*.*`).pipe(
    ftpConnect.dest(`/domains/alexey-morozov.ru/public_html/${repositoryName}`),
  );
}
...
function watcher() {
  ...
  gulpWatch([path.src.allFiles], { usePolling: true }).on(
    "change",
    browserSync.reload,
  );
}

const watchLocal = series(
  parallel(
    pugDist,
    stylesDist,
    stylesLibs,
